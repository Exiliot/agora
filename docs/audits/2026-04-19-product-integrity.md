...
| 9 | medium | Sign-in | `apps/web/src/pages/sign-in/SignInPage.tsx:69` | "Keep me signed in" checkbox is `defaultChecked` with no state and no submission field; session cookie is always 14 days. | Decorative input – users trust it and believe they can toggle persistence. | Either remove the checkbox or wire it to a `rememberMe` flag that shortens the cookie's `maxAge` when unchecked. [CLOSED – see commit below] |
