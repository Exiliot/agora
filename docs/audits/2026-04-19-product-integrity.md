...
| 14 | medium | Deleted-message copy | `apps/web/src/pages/chat/MessageList.tsx:564`, `apps/web/src/ds/MessageRow.tsx:108` | ConfirmModal body promises "Deleting replaces the body with `[deleted]`" but `MessageRow` shows italic `message deleted`. | Inconsistent copy across the confirm and the result; testers notice. | Pick one and use it in both places. [CLOSED in `91ea909`] |
...
Items kept open on purpose – each needs a product or design decision, not more code.

- **#5 (high) – multi-tab focus suppression is keyed per-user, not per-connection.** Honest trade-off: the simpler per-user key is what the notifications spec actually implies ("opening a conversation clears its unread") and it keeps `matches()` O(1). Going per-connection means tracking every tab and OR-ing across the user's connections, plus deciding the "which tab wins on conflicting focus?" edge. Revisit if a user reports a bell entry for a DM they were actively reading on another tab – at that point we have a concrete repro and can price the change against the actual complaint.
- **#21 (low) – native desktop toasts vs requirements drift.** The UI implements `Notification` fallback, the requirements file says desktop push is out of scope. Nothing is broken either way; the choice is "drop the feature" or "amend the spec". Revisit once we know whether the demo judges will sit on the tab or tab-away – if they're going to tab away, the native toast is a demo asset worth keeping. Either action touches documentation, not runtime.
