...
| 20 | low | Dead route | `apps/web/src/App.tsx:44` | `/profile` routes to `ChatView` with no `roomName`, rendering the "pick a room" empty state. | Nothing currently links to it, but a broken link waiting to happen. | Delete the route or wire a real profile page. [CLOSED – see commit below] |
