...
| 15 | medium | Account-level password change | `apps/web/src/pages/**` | `POST /api/auth/password-change` is implemented with rate limit and session-revocation-elsewhere, but no UI calls it. | Logged-in password change (FR-AUTH-10) cannot be exercised from the app. | Add a small form on `/sessions` or a new `/profile` page. [CLOSED – see commit below] |
