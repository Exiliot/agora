...
| 11 | medium | Create room cache | `apps/web/src/features/rooms/useCreateRoom.ts:9` | `onSuccess` invalidates `['rooms']` but not `['conversations']`; `useConversations` still has the stale list. | Newly created room does not appear in the sidebar until another signal (first message) invalidates conversations. | Add `qc.invalidateQueries({ queryKey: ['conversations'] })` alongside the rooms invalidation. [CLOSED – see commit below] |
...
| 18 | low | Sidebar empty state | `apps/web/src/pages/chat/Sidebar.tsx:186-252` | When the user has no rooms and no DMs, the sidebar body is entirely blank below the filter. | First-run user sees nothing in the sidebar except the Create button. | Caption "no rooms yet – join one from Public rooms or create one below." [CLOSED – see commit below] |
