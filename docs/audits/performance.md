...
Scope: full stack; emphasis on the server hot paths and the
chat-scroll client path. Read-only audit; no code changed. This round
focuses on the drift since the previous audit – notifications system,
forgot-password, round-3 design fixes – plus a fresh sweep of areas the
first pass called out as likely to regress.
...
| 5 | high | Integrity | `apps/api/src/messages/ws-handlers.ts:317` | `mark.read` unconditionally overwrites `last_read_message_id` – two out-of-order acks can regress the watermark | Late-arriving `mark.read` older than the current watermark rewinds the read state; unread count goes stale | [CLOSED] `mark.read` UPSERT now carries `setWhere last_read_message_id IS NULL OR last_read_message_id < $new`. UUIDv7 ids sort lexicographically so string compare matches the monotonic semantics. An out-of-order stale ack no longer regresses the watermark. |
...
| 8 | medium | DB | `apps/api/src/messages/routes.ts:226-256` | `/api/conversations` runs 4 sequential queries (memberships, DMs, usernames, unreads, last-read, preview) | Six serial round-trips before response; dominant cost is network RTT, not per-query work | [CLOSED] Memberships + DMs fire in one `Promise.all`; the follow-up usernames, banned-pairs, unread and last-read queries run in a second `Promise.all`. Preview LATERAL still runs serially because it depends on `allIds`. Two RTT instead of six. |
...
| 11 | medium | DB | `apps/api/src/attachments/sweeper.ts:33` | Orphan sweep deletes one attachment row at a time and runs a separate "still referenced" check per hash | At steady state fine; after a refused-upload flood, 1000 orphans do 2000 round-trips | [CLOSED] Sweeper now runs one `DELETE … RETURNING`, one `SELECT` on the unique-hash set for still-referenced rows, and one disk unlink per unique hash. 2000 round-trips collapse to two queries plus parallel unlinks. |
...
| 14 | medium | Client | `apps/web/src/app/WsProvider.tsx:138` | `unread.updated` invalidates every `['conversations']` query and the same for `message.new` at line 88 | On a chatty room, every inbound message triggers a full sidebar refetch | [CLOSED] Both `message.new` and `unread.updated` now pass `refetchType: 'active'` to `invalidateQueries`, so only mounted sidebar consumers refetch. Inactive conversation-list queries stay stale-but-cached until next mount. |
| 15 | medium | Client | `apps/web/src/pages/chat/ChatView.tsx:108` | Members panel renders every `MemberRow` unmemoised; each calls `usePresenceOf` | 1000-member room → 1000 selectors run per `presence.update`; zustand's shallow compare saves render but not selector cost | [CLOSED] `MemberRow` is now `React.memo`-wrapped (its props are stable `{userId, username}` strings). Only the row whose presence actually changed re-runs its selector + re-renders. Virtualisation of the member list is deferred for now; the row count we realistically show (dozens) doesn't warrant it. |
| 16 | medium | Observability | `apps/api/src/server.ts:17` | Pino at `info` in production logs every request and every WS close; debug in dev logs every ws-open / close / hello / heartbeat | Disk churn on a busy night; log volume hides signal | [CLOSED] Request-logging was already disabled in production via `disableRequestLogging`. The ws-plugin connect / disconnect lines were at `debug` and have been demoted to `trace`, so even dev mode (pino `debug`) no longer prints a line on every tab switch. |
...
| 18 | low | Integrity | `apps/api/src/messages/ws-handlers.ts:264` | `message.delete` issues `SELECT ... messages` then `UPDATE ... messages` without a transaction – two users racing delete can both "win" and both publish `message.deleted` | One duplicate `message.deleted` event on concurrent deletes; harmless but noisy | [CLOSED] The delete's UPDATE now carries `WHERE id = $id AND deleted_at IS NULL RETURNING …`. Only the transition null→deletedAt returns a row and publishes; the losing racer gets an idempotent ack with no event. |
